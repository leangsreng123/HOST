...
import { INITIAL_SITES } from './data';
...
export default function App() {
  ...
  // Load sites from localStorage on mount (with short simulated core boot for aesthetic polished feel)
  useEffect(() => {
    const saved = localStorage.getItem('smartbuild_sites');
    let data: Site[] = [];
    if (saved) {
      try {
      ...
      data = INITIAL_SITES;
      localStorage.setItem('smartbuild_sites', JSON.stringify(data));
    }
    ...
    if (data.length > 0) {
      if (siteSlug) {
        ...
        setActiveSite(found || data[0]);
      } else {
        setActiveSite(data[0]);
      }
    }

    // Dynamic elegant boot sequence
  ...
  const saveSites = (updatedSites: Site[]) => {
    ...
    localStorage.setItem('smartbuild_sites', JSON.stringify(updatedSites));
    
    // Sync current active site data with the updated sites list
    if (activeSite) {
      const currentActive = updatedSites.find(s => s.id === activeSite.id);
      setActiveSite(currentActive || (updatedSites.length > 0 ? updatedSites[0] : null));
    } else if (updatedSites.length > 0) {
      setActiveSite(updatedSites[0]);
    } else {
      setActiveSite(null);
    }
  };
  ...
  const handleUpdateSite = async (updatedSite: Site) => {
    ...
    const updated = sites.map(s => s.id === cleanSite.id ? cleanSite : s);
    saveSites(updated);
  };
  ...
  const handleSubmitRequest = async (slug: string, reqData: Omit<ProjectRequest, 'id' | 'createdAt' | 'status'>) => {
    const updated = sites.map(site => {
      if (site.slug === slug) {
        ...
        return {
          ...site,
          projectRequests: [newRequest, ...(site.projectRequests || [])]
        };
      }
      return site;
    });
  ...
  const handleJoinDirectory = async (slug: string, contractorData: Omit<Contractor, 'id' | 'rating' | 'reviewsCount'>) => {
    const updated = sites.map(site => {
      if (site.slug === slug) {
        ...
        return {
          ...site,
          contractors: [...(site.contractors || []), newContractor]
        };
      }
      return site;
    });
  ...
  // sync active site view when slug changes inside SPA custom route
  useEffect(() => {
    if (siteSlug && sites.length > 0) {
      const found = sites.find(s => s.slug === siteSlug);
      if (found) {
        setActiveSite(found);
      }
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-xs text-slate-400">Loading custom themes, contractor listings, and local database engine.</p>
      </div>
    );
  }
      ...
      <header id="wp-topbar" className="bg-slate-900 border-b border-slate-800 px-6 py-3 flex items-center justify-between shadow-md">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-400 font-mono">Status: Local Client-Side Database Engine Running Offline Forever</p>
          </div>
        </div>
        ...
        {activeSite && (
          <div className="flex items-center space-x-3">
            ...
            <button
              id="view-live-btn"
              onClick={() => navigateTo(`/site/${activeSite.slug}`)}
